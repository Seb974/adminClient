...
function findAll() {
    return api
        .get('/api/parent_departments')
        .then(response => response.data['hydra:member'].sort((a, b) => (a.name > b.name) ? 1 : -1));
}

function findAllPaginated(page = 1, items = 30) {
    return api
        .get(`/api/parent_departments?&order[name]=asc&pagination=true&itemsPerPage=${ items }&page=${ page }`)
...
function findWord(word, page = 1, items = 30) {
    return api
        .get(`/api/parent_departments?name[]=${ word }&order[name]=asc&pagination=true&page=${ page }&itemsPerPage=${ items }`)
        .then(response => response.data)
        .catch(error => []);
}

function deleteDepartment(id) {
    return api.delete('/api/parent_departments/' + id);
}

function find(id) {
    return api
        .get('/api/parent_departments/' + id)
        .then(response => response.data);
}

function update(id, department) {
    return api.put('/api/parent_departments/' + id, {...department});
}

function create(department) {
    return api.post('/api/parent_departments', {...department});
}
