...
function findGroupsWithShopAccess() {
    return api
        .get('/api/groups?hasShopAccess=true&order[label]=asc')
        .then(response => response.data['hydra:member'].map(g => ({...g, isFixed: false})));
}
